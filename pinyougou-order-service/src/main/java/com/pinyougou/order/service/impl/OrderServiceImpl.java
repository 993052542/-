...
public class OrderServiceImpl implements OrderService {
	...
	public void add(TbOrder order) {
		...
		for (Cart cart : cartList) {
			...
			tbOrder.setUserId(order.getUserId());//购买用户的id
			tbOrder.setReceiverAreaName(order.getReceiverAreaName());//收货地址
			tbOrder.setReceiverMobile(order.getReceiverMobile());//收货人的手机
			tbOrder.setReceiver(order.getReceiver());//收货人的名字
			tbOrder.setSourceType(order.getSourceType());//订单的来源
			tbOrder.setSellerId(cart.getSellerId());//商家的id
			
			double money = 0;//订单总金额
			//循环购物车中的每条明细记录
		...
		System.out.println("当前用户的购物车已从redis清空");
